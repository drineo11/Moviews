<?php
	...
	class UserDAO implements UserDAOInterface {
		...
		public function buildUser($data) {
			...
			$user -> updateCode = $data["use_update"];
		...
		public function create(User $user, $authUser = false) {
			$stmt = $this -> conn -> prepare("insert into users (use_name, use_lastname, use_email, use_password, use_update, use_token) values (:use_name, :use_lastname, :use_email, :use_password, :use_update, :use_token)");
			...
			$stmt -> bindParam(":use_update", $user -> updateCode);
		...
		public function setTokenToSession($token, $redirect = true) {
			...
			if ($redirect) $this -> message -> setMessage("Seja bem-vindo!", "success", "editprofile.php");
		}
		
		public function updatePassword($code, $new_password) {
			$stmt_01 = $this -> conn -> prepare("select use_email from users where use_update = :use_update");
			
			$stmt_01 -> bindParam(":use_update", $code);
			$stmt_01 -> execute();
			
			if ($stmt_01 -> rowCount() > 0) {
				$user = new User();
				$data = $stmt_01 -> fetch();
				
				$password_encrypt = $user -> generatePassword($new_password);
				$new_code = $user -> generateUpdateCode();
				$email_temp = $data["use_email"];
				
				$stmt_02 = $this -> conn -> prepare("update users set use_password = :use_password, use_update = :use_update where use_email = :use_email");
				
				$stmt_02 -> bindParam(":use_password", $password_encrypt);
				$stmt_02 -> bindParam(":use_update", $new_code);
				$stmt_02 -> bindParam(":use_email", $email_temp);
				$stmt_02 -> execute();
		...
		public function changePassword(User $user) {
			$stmt = $this -> conn -> prepare("update users set use_password = :use_password, use_update = :use_update where use_id = :use_id");
