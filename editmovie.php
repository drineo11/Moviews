<?php
	...
	if (empty($id)) $message -> setMessage("O filme não foi encontrado!", "error", "index.php");
	else {
		...
		<div class="row">
			<div class="col-md-6 offset-md-1">
				...
				<p class="page-description">
					Altere os dados do filme no fomrulário abaixo:
				</p>
				<form id="edit-movie-form" action="<?= $BASE_URL ?>movie_process.php" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<select name="category" id="category" class="form-control">
							...
							<option value="Ação" <?= $movie -> category === "Ação" ? "selected" : "" ?>>
								Ação
							</option>
							<option value="Drama" <?= $movie -> category === "Drama" ? "selected" : "" ?>>
								Drama
							</option>
							<option value="Comédia" <?= $movie -> category === "Comédia" ? "selected" : "" ?>>
								Comédia
							</option>
							<option value="Fantasia / Ficção" <?= $movie -> category === "Fantasia / Ficção" ? "selected" : "" ?>>
								Fantasia / Ficção
							</option>
							<option value="Romance" <?= $movie -> category === "Romance" ? "selected" : "" ?>>
								Romance
							</option>
						</select>
					</div>
